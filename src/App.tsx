...
export default function App() {
  ...
  const content = (() => {
    ...
    if (screen === 'mission-template') {
      ...
      return progress ? <MissionTemplate mission={selectedMission} user={{ ...user, displayName: progression?.hackerCodename ?? 'ANONYMOUS' }} progress={progress} progression={progression} onHome={() => { setAttemptId(undefined); setScreen('home'); }} onRewardContinue={pendingIdentity || pendingTransmission ? () => setScreen('home') : undefined} onShop={() => setScreen('shop')} onKeyboard={() => { const next = dashboard.missions.find(m => m.missionId === 'mission-3'); if (next?.unlocked) { setSelectedMissionId('mission-3'); } else { setSelectedRobotDefenseMode('robot_override'); setRobotDefenseReturnScreen('home'); setScreen('robot-defense'); } }} onNextMission={() => { const next = dashboard.missions.find(m => m.missionNumber === selectedMission.number + 1 && m.unlocked); if(next){setAttemptId(undefined);setSelectedMissionId(next.missionId);}else setScreen('home'); }} onAttemptChange={setAttemptId} onComplete={async () => setDashboard(await api<StudentDashboard>('student.dashboard', {}, user.csrfToken))} /> : null;
    }
