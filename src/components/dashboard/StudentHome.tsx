...
export function StudentHome({ user, dashboard, onMission, onSettings }: { user: SessionUser; dashboard: StudentDashboard; onMission: (missionId: string) => void; onSettings: () => void }) {
  ...
  return <main className="page home-page"><section className="welcome-strip"><div><p className="eyebrow">AGENT HOME / LEVEL {String(dashboard.currentMission).padStart(2, '0')}</p><h1>{copy.welcome.en}</h1><small className="home-support-heading" lang={copy.welcome.lang}>{copy.welcome.support}</small><div className="bilingual"><div>{copy.nextSkill.en}</div><small lang={copy.nextSkill.lang}>{copy.nextSkill.support}</small></div></div><div className="rank-badge"><span>{copy.currentMission.en}</span><small lang={copy.currentMission.lang}>{copy.currentMission.support}</small><strong>Mission {dashboard.currentMission}</strong><span>{copy.totalPoints.en}</span><small lang={copy.totalPoints.lang}>{copy.totalPoints.support}</small><strong>{dashboard.totalPoints}</strong></div></section>
