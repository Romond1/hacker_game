...
try {
    $result = [
        ...
        'graduates' => $pdo->query("SELECT COUNT(*) FROM (SELECT user_id FROM user_progress WHERE completed = 1 AND mission_id IN ('mission-1','mission-2','mission-3') GROUP BY user_id HAVING COUNT(*) = 3) graduates")->fetchColumn(),
    ];
    $pdo->rollBack();
